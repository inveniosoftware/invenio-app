...
def test_limiter_for_authenticated_user(app, create_mocked_flask_login_with_user_init):
    """Test the Flask limiter function."""
    init_mocked_flask_login_with_user = create_mocked_flask_login_with_user_init
    init_mocked_flask_login_with_user({"is_authenticated": True, "id": "1825"})
    with app.test_client() as client:
        exhaust_and_test_rate_limit_per_second(
...
def test_limiter_for_privileged_user(
    app, create_mocked_flask_login_with_user_init, push_rate_limit_to_context
):
    """Test the Flask limiter function."""
    init_mocked_flask_login_with_user = create_mocked_flask_login_with_user_init
    user = {"is_authenticated": True, "id": "1111"}
    init_mocked_flask_login_with_user(user)
    with app.test_client() as client:
